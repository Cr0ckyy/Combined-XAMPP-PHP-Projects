...
if (!empty($_POST)) {
        ...
        if (!empty($fieldValue)) {
            switch ($fieldKey) {
                case "name":
                    ...
                    if (empty($MailChimpContact["FNAME"]) && empty($_POST["firstname"]))
                        $MailChimpContact["FNAME"] = $fieldValue;
                    if (empty($sendInBlueContact["attributes"]["FIRSTNAME"]) && empty($_POST["firstname"]))
                        $sendInBlueContact["attributes"]["FIRSTNAME"] = $fieldValue;
                    if (empty($hubSpotContact["firstname"]) && empty($_POST["firstname"]))
                        $hubSpotContact["firstname"] = $fieldValue;
                    continue;
                case "firstname":
                    $message .= "<b>First Name:</b> " . $fieldValue . '<br />';
                    if (empty($MailChimpContact["FNAME"]))
                        $MailChimpContact["FNAME"] = $fieldValue;
                    if (empty($sendInBlueContact["attributes"]["FIRSTNAME"]))
                        $sendInBlueContact["attributes"]["FIRSTNAME"] = $fieldValue;
                    if (empty($hubSpotContact["firstname"]) && empty($_POST["firstname"]))
                        $hubSpotContact["firstname"] = $fieldValue;
                    continue;
    ...
    if (!empty($MailChimpAPIkey) && !empty($MailChimpListID) && !empty($_POST['email'])) {
        ...
        if (!empty($MailChimpContact["ADDRESS"])) {
            ...
            if (empty($MailChimpContact["ADDRESS"]["addr1"]))
                $MailChimpContact["ADDRESS"]["addr1"] = 'Address not set';
            if (empty($MailChimpContact["ADDRESS"]["city"]))
                $MailChimpContact["ADDRESS"]["city"] = 'City not set';
            if (empty($MailChimpContact["ADDRESS"]["state"]))
                $MailChimpContact["ADDRESS"]["state"] = 'State not set';
            if (empty($MailChimpContact["ADDRESS"]["zip"]))
                $MailChimpContact["ADDRESS"]["zip"] = 'ZIP not set';
        }
