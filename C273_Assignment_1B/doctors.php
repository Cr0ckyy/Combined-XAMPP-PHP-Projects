...
<script>
    ...
    $('.view_schedule').click(function () {
        my_modal($(this).attr('data-name') + " - Schedule", "view_doctor_schedule.php")
    })
    $('.set_appointment').click(function () {
        if ('<?php echo isset($_SESSION['login_id']) ?>' == 1)
            my_modal("Set Appointment with " + $(this).attr('data-name'), "set_appointment.php")
        else {
            my_modal("Login First", "login.php")
        }
    })
</script>
